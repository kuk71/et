...
class StudyKit extends Model
{
    ...
    /** @var bool Определяет какие коллекции выводятся публичные или конкретного зарегистрированного пользователя */
    public $isPublicKits = false;

    /**
     * @var null содежит текст ошибки если = null - ошибок нет
     */
    public $errorText = null; // содержит текст ошибки // если = null - ошибок нет
    ...
    {
        try {
            if ($this->isPublicKits) {
                $userId = \Yii::$app->params['idOwnerPublicKit'];
            } else {
    ...
    {
        // проверить коллекции на принадлежность текущему пользователю или к публичной группе коллекций
     ...
     * @param $kitId
     */
    public function validateKitId($kitIdList)
    {
        foreach ($kitIdList as $kitId) {
            UserAccess::redirectIfKitNotUserOrPublic($kitId);
        }
    }
}