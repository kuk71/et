 ...
 * @property int $study_kit_created_timestamp Таймстамп UTC времени создания коллекции. Сделано для независимости времени от часового пояса установленного в MySql и PHP.
 */
class StudyKitList extends ActiveRecordAlias
{
    ...
    /**
     * Удаляет коллекцию и все не использующиеся больше словарные статьи из БД
     *
     * @param $kitId
     */
    ...
    /**
     * Пытается найти у пользователя $userId коллекцию для изучения $kitId
    ...
    /**
     * Пытается найти у пользователя $userId коллекцию для изучения $kitId
     * или проверить эту коллекцию на принадлежность к публичной группе коллекций
     * если коллекция не найдена, возвращает true, найдена false
     *
     * @param $userId
     * @param $kitId
     * @return bool
     */
    public static function isNotFindKitUserOrPublic($userId, $kitId)
    {
        $KitListArray = self::findOne([
            'study_kit_id' => $kitId,
            'user_id' => [$userId, \Yii::$app->params['idOwnerPublicKit'],]]);
    ...
    /**
     * Возвращает имя и описание коллекции слов с id = $kitId
     *
     * @param $kitId - id коллекции слов
     * @return array|ActiveRecordAlias[]
     */
    public static function getStudyKitName($kitId): array
    {
        return self::find()
            ->select([
                'study_kit_name',
