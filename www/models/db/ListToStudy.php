...
class ListToStudy extends \yii\db\ActiveRecord
{
    ...
    public static function getDicEntryContent($kitId, $userId)
    {
        ...
        $kit = self::findBySql("
            ...
            WHERE
                ...
                AND (study_kit_list.user_id = $userId OR study_kit_list.study_kit_public = 1)
    ...
    /**
     * Возвращает массив из id иностранных слов добавленных в коллекцию $kitId
     *
     * @param int $kitId
     * @return array
     */
    public static function getWordIdFromKit(int $kitId)
    {
    ...
    private static function convertArrayKitForStudy($entryList)
    {
        ...
        foreach ($entryList as $entry) {
            ...
            $kit[$entry['dic_entry_id']][0] = $entry['word_id_1'];
            $kit[$entry['dic_entry_id']][1] = $entry['word_spelling_1'];
            $kit[$entry['dic_entry_id']][2][] = $entry['word_spelling_2'];
        }
