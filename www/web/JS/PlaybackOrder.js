...
// аудио проигрыватель для повторного воспроизведения одного и того же слова
const playerReplay = new Audio();

playerReplay.onended = function() {
    setTimeout(replay, 1500);
};

// признак, что авто повторение выключено
let playerReplayOff = true;
let playerReplayWordId =-1;

// запускает повторение иностранного слова
function replay() {
    if (playerReplayOff) {
        document.getElementById('btnRePlay').innerHTML = 'Повторять слово';
        return;
    }

    document.getElementById('btnRePlay').innerHTML = 'Остановить повтор';

    // останавливает основное воспроизведение если оно включено
    if (!playbackPause) {
        play();
    }

    playerReplay.src = mp3Name(playerReplayWordId);
    playerReplay.play();
}
...
function play() {
    ...
    if (playbackPause) {
        ...
        playerReplayOff = true; // выключает авто повторение слова
    }
    ...
    if (!playerReplayOff) {
        // останавливает повторение слова
        playerReplayOff = true;
    }
...
function playBlockElement() {
    ...
    player.src = mp3Name(block[blockPosition]['wordId']);
    ...
    if (block[blockPosition]['wordType'] == 'word') {
        ...
        playerReplayWordId = block[blockPosition]['wordId'];
    } else {
...
// возвращает URL клипа по id слова
function mp3Name(id) {
...
function createBlock(direction) {
    ...
    // открыть возможность повторять слово
    document.getElementById('btnRePlay').style.display = '';

    return true;
}

// получает из текствого поля с id = wordInBlock сколько нужно включить слов в блок
